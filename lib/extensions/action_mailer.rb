class ActionMailer::Base
  ...
  def deliver_with_rescue_errors!(mail = @mail)
    begin
      ...
      message = Base64::encode64(Marshal.dump({:mail_yaml => mail.to_yaml, :mailer_name => self.class.name}))
  ...
  def deliver_with_receipt!(mail = @mail)
    ...
    unless (self.delivery_method == :smtp && self.smtp_settings[:address] == 'smtp.sendgrid.net')
      if mail.bcc
