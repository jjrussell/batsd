...
describe Device do
  ...
  describe '.lookup_by_device_info' do
    before :each do
      @params = {
        :udid             => 'test_udid',
    ...
    context 'tries to find the device based on params' do
      it 'tries to find each of the provided params' do
        DeviceIdentifier.should_receive(:find_device_from_params).with(@params).and_return(nil)
        Device.lookup_by_device_info(@params).should == nil
      end

      it 'returns the right device when it finds it' do
        device = FactoryGirl.create(:device)
        DeviceIdentifier.should_receive(:find_device_from_params).with(@params).and_return(device)
        Device.lookup_by_device_info(@params).should == device
      end

      context 'when the advertising id changes' do
        it 'returns a new device' do
          device = FactoryGirl.create(:device, :advertising_id => 'old_test_advertising_id', :mac_address => 'some_mac')
          DeviceIdentifier.should_receive(:find_device_from_params).with(@params).and_return(device)

          new_device = Device.lookup_by_device_info(@params)
          new_device.key.should_not == device.key
          new_device.new_record?.should be_true
          new_device.mac_address.should == 'some_mac'
        end
      end
    end
  end
