...
describe PingerDataController do
  describe '#index' do
    before :each do
      @controller.should_receive(:pinger_authenticate).at_least(:once).and_return(true)
    end
    ...
    context 'valid params' do
      before :each do
        ...
        partner.stub(:offers).and_return('pinger_offers')
        Appstats.stub(:new).and_return('pinger_test')
      end

      it 'should render the shared/publisher_data template' do
        get(:index, @params)
        response.should render_template('shared/publisher_data')
      end
      ...
      it 'has instance variable @appstats_list' do
        get(:index, @params)
        assigns(:appstats_list).should == [['pinger_offers', 'pinger_test']]
      end
    end
  end
end
