...
describe BillingController do
  ...
  describe '#update_payout_info' do
    before :each do
      ...
      @partner.payout_info_confirmation.confirmed = true
      @partner.save!
    ...
    context 'when payouts are already confirmed for the partner' do
      it 'unconfirms payouts' do
        @partner.payout_info_confirmation.confirmed.should be_false
      end

      it 'adds a system not that the payout info changed' do
        @partner.payout_info_confirmation.system_notes.should == "SYSTEM: Partner Payout Information has changed."
      end
    end
  end
end
