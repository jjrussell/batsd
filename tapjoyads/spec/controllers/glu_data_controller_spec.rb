...
describe GluDataController do
  describe '#index' do
    before :each do
      @controller.should_receive(:glu_authenticate).at_least(:once).and_return(true)
    end
    ...
    context 'valid params' do
      before :each do
        ...
        partner.stub(:offers).and_return('glu_offers')
        Appstats.stub(:new).and_return('glu_test')
      end
      ...
      it 'has instance variable @appstats_list' do
        get(:index, @params)
        assigns(:appstats_list).should == [['glu_offers', 'glu_test']]
      end
    end
  end
end
