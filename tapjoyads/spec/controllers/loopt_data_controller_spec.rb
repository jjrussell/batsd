...
describe LooptDataController do
  describe '#index' do
    before :each do
      @controller.should_receive(:loopt_authenticate).at_least(:once).and_return(true)
    end
    ...
    context 'valid params' do
      before :each do
        ...
        partner.stub(:offers).and_return('loopt_offers')
        Appstats.stub(:new).and_return('loopt_test')
      end
      ...
      it 'has instance variable @appstats_list' do
        get(:index, @params)
        assigns(:appstats_list).should == [['loopt_offers', 'loopt_test']]
      end
    end
  end
end
