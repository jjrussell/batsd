...
describe Dashboard::CurrenciesController do
  ...
  describe '#show' do
    context 'non-rewarded currency' do
      before :each do
      ...
      it 'redirect to non rewarded index' do
        response.should redirect_to(app_non_rewarded_index_path(:app_id => @app.id))
      end
    end
    context 'rewarded currency' do
      before :each do
        @app = FactoryGirl.create(:app)
  ...
  describe '#update' do
    ...
    context 'rewarded currency' do
      ...
      context 'successful update' do
        ...
        it 'redirect to non rewarded index' do
          response.should redirect_to(app_currency_path(:app_id => @app.id, :id => @currency.id))
        end
      end
      context 'unsuccessful update' do
        before :each do
