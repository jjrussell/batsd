...
describe SprocketHelper do
  ...
  context "with combine set to false" do
    before :each do
      SPROCKETS_CONFIG[:combine] = false
      Sprockets::Tj.combine = false
    end
  ...
  context "with combine set to true" do
    before :each do
      SPROCKETS_CONFIG[:combine] = true
      Sprockets::Tj.combine = true
    end
    describe "#js_tag" do
