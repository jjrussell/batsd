class OfferTriggeredActionsController < ApplicationController
  ...
  def fb_login
    @redirect_url = "#{WEBSITE_URL}/gamer/create_account_for_offer?tapjoy_device_id=#{params[:tapjoy_device_id]}"
  end
  ...
  def setup
    return unless verify_params([ :data, :id, :tapjoy_device_id, :publisher_app_id ])
