class Apps::Offers::OfferEventsController < WebsiteController
  layout 'tabbed'
  current_tab :apps
  ...
  def index
    ...
    @offer_events = @offer.offer_events.send(@event_scope).paginate(:page => params[:page], :per_page => 40)
  end

  def new
    @offer_event = @offer.events.build
  end
