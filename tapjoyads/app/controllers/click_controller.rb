class ClickController < ApplicationController
  ...
  def setup
    ...
    if APPS_WITH_BAD_PUB_USER_ID.include?(params[:publisher_app_id])
      params[:publisher_user_id] = params[:tapjoy_device_id]
    end
  end
  ...
  def create_click(type)
    ...
    click.tapjoy_device_id       = params[:tapjoy_device_id]
    ...
    @offer.queue_click_tracking_requests(params.slice(:tapjoy_device_id, :publisher_app_id).merge(:ip_address => ip_address))
  end
  ...
  def destination_url
    @offer.destination_url({
      :udid                  => params[:udid],
      :tapjoy_device_id      => params[:tapjoy_device_id],
  ...
  def click_key
    @click_key ||= @offer.format_as_click_key(params.slice(:tapjoy_device_id, :advertiser_app_id, :gamer_id))
  end
