class GamesController < ApplicationController
  ...
  def current_device_id
    if session[:current_device_id]
      @current_device_id = ObjectEncryptor.decrypt(session[:current_device_id], SYMMETRIC_CRYPTO_SECRET)
    else
    ...
    session[:current_device_id] ||= ObjectEncryptor.encrypt(@current_device_id, SYMMETRIC_CRYPTO_SECRET)
    @current_device_id
  end
  ...
  def current_device_id_cookie
    if cookies[:data]
      begin
        cookie_data = ObjectEncryptor.decrypt(cookies[:data], SYMMETRIC_CRYPTO_SECRET)
        cookie_data[:udid]
      rescue
        nil
      end
  ...
  def set_current_device(data)
    device_data = ObjectEncryptor.decrypt(data, SYMMETRIC_CRYPTO_SECRET)
    if valid_device_id(device_data[:udid])
      session[:current_device_id] = ObjectEncryptor.encrypt(device_data[:udid], SYMMETRIC_CRYPTO_SECRET)
      session[:current_device_id] ? ObjectEncryptor.decrypt(session[:current_device_id], SYMMETRIC_CRYPTO_SECRET) : nil
    end
  end
