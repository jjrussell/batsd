class ReengagementRewardsController < ApplicationController
  ...
  def show
    ...
    @currencies = @app.currencies if @app
    @reengagement_offers = @app.reengagement_campaign if @currencies
  ...
  def index
    ...
    render :nothing => true, :status => 204 and return unless @app.try(:reengagement_campaign_enabled?) && @reengagement_offers
  end
