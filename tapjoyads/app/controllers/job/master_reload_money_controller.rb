  ...
  def get_money_stats(interval_strings)
    Conversion.using_slave_db do
      ...
      interval_strings.keys.each do |is|
        ...
        publisher_earnings = Conversion.sum(:publisher_amount, :conditions => [ "created_at > #{interval_strings[is]} AND publisher_app_id NOT IN (?)", tj_partner.app_ids ]) / 100.0
        ...
        linkshare_est = conversions * 0.0104
