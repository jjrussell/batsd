class Job::QueueCalculateShowRateController < Job::SqsReaderController
  ...
  def on_message(message)
    ...
    if recent_clicks > 200 && conversion_rate < min_conversion_rate
      Notifier.alert_new_relic(ConversionRateTooLowError, "#{offer.name_with_suffix} (https://www.tapjoy.com/statz/#{offer.id}) has #{conversion_rate} cvr on #{recent_clicks} clicks.", nil, { :partner_id => offer.partner.id })
    end
