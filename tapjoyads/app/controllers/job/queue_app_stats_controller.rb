...
private
  ...
  def verify_yesterday
    ...
    if stat_row.get_hourly_count('paid_clicks').sum > 0
      app_condition = "app_id = '#{@offer.id}'"
      Stats::TOP_COUNTRIES.each do |country|
        stat_name = ['country_conversions', "paid_clicks.#{country}"]
