class Sales::CreateAccountController < Sales::SalesController
  ...
  def confirm
    unless verify_params([:sales_person_id, :email, :password, :company_name, :contact_name,
        :contact_email, :contact_phone, :free_credits], 
  ...
  def create
    ...
    url = 'http://ws.tapjoyads.com/Service1.asmx/CreateAccount?sales_password=uew862nvm01ds' +
    ...
    # else
    #   flash[:error] = "Failed to make account for #{params[:email]}"
    # end
