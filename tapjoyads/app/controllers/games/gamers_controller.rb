class Games::GamersController < GamesController
  ...
  def create
    ...
    if @gamer.save
      message = { :gamer_id => @gamer.id, :accept_language_str => request.headers['accept-language'], :user_agent_str => request.headers['user-agent'], :using_android => using_adroid? }.to_json
