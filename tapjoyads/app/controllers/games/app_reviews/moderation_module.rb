module Games::AppReviews::ModerationModule
  def create
    gamer_review = current_gamer.app_reviews.find_by_id(params[:app_review_id])
    ...
    when current_gamer.review_moderation_votes.exists?(:app_review_id => params[:app_review_id])
      render :json => { :msg => "Can't flag different ways'" }, :status => 403
