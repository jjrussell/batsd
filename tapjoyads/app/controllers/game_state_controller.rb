class GameStateController < ApplicationController
  ...
  def save
    return unless verify_params([:app_id, :publisher_user_id, :data, :game_state_points]) && verify_records(get_device_key)
    @game_state = GameState.new :key => "#{params[:app_id]}.#{params[:publisher_user_id]}"
    @game_state.data = params[:data]
    @game_state.version += 1
    @game_state.add_device(get_device_key)
...
private
  ...
  def get_mapping
    if params[:publisher_user_id].blank?
      mapping = GameStateMapping.new :key => "#{params[:app_id]}.#{get_device_key}"
