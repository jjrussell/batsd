class GameStateController < ApplicationController
  ...
  def load
    return unless verify_params([:app_id, :tapjoy_device_id, :publisher_user_id])
  ...
  def save
    return unless verify_params([:app_id, :publisher_user_id, :data, :tapjoy_device_id, :game_state_points])
    @game_state = GameState.new :key => "#{params[:app_id]}.#{params[:publisher_user_id]}"
    @game_state.data = params[:data]
    @game_state.version += 1
    @game_state.add_device params[:tapjoy_device_id]
...
private
  ...
  def get_mapping
    if params[:publisher_user_id].blank?
      mapping = GameStateMapping.new :key => "#{params[:app_id]}.#{params[:tapjoy_device_id]}"
