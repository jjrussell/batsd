module ToolsHelper
  ...
  def click_info_ul(click, reward)
    ...
    concat_li("Tapjoy Device ID's for blocking", click.publisher_user_tapjoy_device_ids.join('<BR/>')) if click.block_reason =~ /TooManyTapjoyDeviceIDsForPublisherUserId/
  ...
  def click_tr_class(click, reward)
    ...
    if click.block_reason =~ /TooManyTapjoyDeviceIDsForPublisherUserId|Banned/
      classes << 'blocked'
