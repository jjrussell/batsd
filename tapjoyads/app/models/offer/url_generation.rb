module Offer::UrlGeneration
  ...
  def display_ad_image_url(publisher_app_id, width, height, currency_id = nil, display_multiplier = nil, bust_cloudfront_cache = false)
    size = "#{width}x#{height}"

    if display_custom_banner_for_size?(size)
      url = "#{CLOUDFRONT_URL}/#{banner_creative_path(size)}"
      url << "?ts=#{Time.now.to_i}" if bust_cloudfront_cache
    else
