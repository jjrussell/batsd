class PressRelease < ActiveRecord::Base
  ...
  def link
    self.content.blank? ? link_href : "/press/#{link_href}"
  end

  def content
    date = published_at.strftime("%B %e, %Y")
    press_date = "<strong>San Francisco, CA &ndash; #{date}</strong> &ndash; "
    body = content_body
    body.gsub!('- press_date', press_date)
    body.gsub!(/- about_blurb.*$/, about_blurb)
