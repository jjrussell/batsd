...
class Offer < ActiveRecord::Base
  ...
  def save_icon!(icon_src_blob)
    ...
    if item_type == 'App' && app.present? && app.primary_app_metadata.present?
      ...
      bucket.objects["icons/256/#{meta_icon_id}.jpg"].write(:data => icon_256_blob, :acl => :public_read)
      bucket.objects["icons/114/#{meta_icon_id}.jpg"].write(:data => icon_114_blob, :acl => :public_read)
      bucket.objects["icons/57/#{meta_icon_id}.jpg"].write(:data => icon_57_blob, :acl => :public_read)
      bucket.objects["icons/57/#{meta_icon_id}.png"].write(:data => icon_57_png_blob, :acl => :public_read)
      bucket.objects["icons/src/#{meta_icon_id}.jpg"].write(:data => icon_src_blob, :acl => :public_read)
