class Device < SimpledbShardedResource
  ...
  ALL_IDENTIFIERS = [:open_udid, :android_id, :advertising_id, :udid, :mac_address]
  ...
  def self.lookup_by_device_info(params)
    device = DeviceIdentifier.find_device_from_params(params)

    if device.present? && params.key?(:advertising_id) && device.advertising_id? && device.advertising_id != params[:advertising_id]
      new_device = Device.new
      new_device.advertising_id = device.advertising_id
  ...
  def self.define_lookup_identifiers
    ALL_IDENTIFIERS.each do |identifier|
      define_method "#{identifier.to_s}=" do |new_value|
        ...
        @create_device_identifiers ||= ("#{identifier.to_s}" != new_value)
        put("#{identifier.to_s}", new_value)
      end
    end
  end
  ...
  def handle_connect!(app_id, params)
    ...
    ALL_IDENTIFIERS.each do |identifier|
      self.send("#{identifier.to_s}=", params[identifier]) if params[identifier].present?
    end
  ...
  def create_identifiers!
    ...
    unless has_tapjoy_id?
      all_identifiers << Digest::SHA2.hexdigest(self.key)
      all_identifiers << Digest::SHA1.hexdigest(self.key)
    end
    ...
    all_identifiers.each do |identifier|
      ...
      next if device_identifier.device_id == key
