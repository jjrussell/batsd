class SupportRequest < SimpledbResource
  ...
  def fill(options)
    ...
    gamer_device  = options.delete(:device)     { nil }
    ...
    device = gamer_device.present? ? Device.find_by_device_id(gamer_device.device_id) : nil
    ...
    self.udid                   = device.present? && device.is_a?(Device) ? device.udid : params[:udid]
    self.tapjoy_device_id       = device.present? && device.is_a?(Device) ? device.key : params[:tapjoy_device_id]
    self.advertising_id         = device.present? && device.is_a?(Device) ? device.advertising_id : params[:advertising_id]
    self.mac_address            = device.present? && device.is_a?(Device) ? device.mac_address : params[:mac_address]
    self.device_type            = gamer_device.present? ? gamer_device.device_type : params[:device_type]
