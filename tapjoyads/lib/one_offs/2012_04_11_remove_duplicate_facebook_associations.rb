class OneOffs
  def self.remove_duplicate_facebook_associations
    results = GamerProfile.find_by_sql("SELECT facebook_id, COUNT(*) AS acct_count FROM gamer_profiles WHERE facebook_id IS NOT NULL GROUP BY facebook_id")
    results.each do |result|
      if result.acct_count.to_i > 1
        Gamer.find(
          :all,
