class Experiments
  ...
  def self.get_experiment_data(start_time, end_time, experiment_id)
    ...
    while date <= end_time.to_date + 2.days && date <= Time.zone.now.to_date      
      offerwall_views += VerticaCluster.count 'web_request', "path LIKE '%offers%' AND #{viewed_at_condition_vertica} AND exp = '#{experiment_id}'"
      clicks += VerticaCluster.count 'web_request', "path LIKE '%offer_click%' AND #{viewed_at_condition_vertica} AND exp = '#{experiment_id}'"
      conversions += VerticaCluster.count 'web_request', "path LIKE '%conversion%' AND #{viewed_at_condition_vertica} AND exp = '#{experiment_id}'"
      date += 1.day
    end
