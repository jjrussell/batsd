class OfferPrerequisitesValidator < ActiveModel::Validator
  def validate(record)
    if record.prerequisite_offer_id.present?
      ...
      record.errors[:prerequisite_offer_id] << 'prerequisite offer can not be the same as exclusion prerequisite offer.' if record.get_exclusion_prerequisite_offer_ids.include?(record.prerequisite_offer_id)
    end
