...
>>>>>>> 1d70261625ee91f154994e8ec91eddcef13815d4
    }
    ...
    showMoreGames : function() {
      ...
      if (TJG.moreAppOfferWall) {
        ...
        $.ajax({
          ...
          error: function() {
            var msg = _t('games.generic_issue');
            $("#more_games_content").html('<div>'+msg+'</div>').fadeIn(TJG.ui.fadeSpd);
          }
        });
      }
    },
      ...
      if (url) {
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if (data.OfferArray) {
              ...
              if (TJG.appOfferWall[appId]['offers_left'] > 0) {
                t.push('<div class="more_button_wrapper"><div class="get_more_apps" app_id="' + appId + '"><div class="get_more_apps_content">' + _t('games.load_more')+'</div></div></div>');
              }
              else {
                t.push('<div class="more_button_wrapper"><div class="back_to_top grey_button"><div class="grey_button_content">'+_t('games.back_to_top')+'</div></div></div>');
              ...
              $(".get_more_apps").click(function(){
                ...
                if (TJG.appOfferWall[appId]['offers_left'] > 0) {
                  ...
                  $.ajax({
                    ...
                    success: function(data) {
                      if (data.OfferArray) {
                        ...
                        if (TJG.appOfferWall[appId]['offers_left'] > 0) {
                          ...
                          $(".more_button_wrapper").html('<div class="back_to_top grey_button"><div class="grey_button_content">'+_t('games.back_to_top')+'</div></div>');
                          $(".back_to_top").click(function(){
                            TJG.utils.scrollTop();
                          });
                        }
                    ...
                    error: function () {
                      var msg = _t('games.cant_fetch_offers'),
                        m = [
                      ...
                      hasFailed = true;
                      $(".get_more_apps_content").html(_t('games.load_more'));
          ...
          error: function() {
            var msg = _t('games.generic_issue');
            TJG.ui.hideLoader();
            $("#earn_content").html('<div class="center">'+msg+'</div>').fadeIn(fadeSpd);
            TJG.utils.scrollTop();
          }
        });
      }
            ...
            t.push('<div class="offer_text">');
              ...
              if (v.Type && v.Type == 'App') {
                t.push('<div class="offer_install">');
                  t.push(_t('games.install_and_run', {name: v.Name}));
                t.push('</div>');
              }
              ...
              if (v.WifiOnly) {
                t.push('<div class = "wifi_only">');
                  t.push(_t('games.wifi_required')+' <div class="wifi_icon"></div>');
                t.push('</div>');
              }
            t.push('</div>');
          t.push('</li>');
    ...
    showRegister : function () {
      ...
      $('form#new_gamer').submit(function(e){
        ...
        if(values['date[day]'] == '' || values['date[month]'] == '' || values['date[year]'] == '') {
          $(".email_error").html(_t('games.enter_birthdate'));
          hasError = true;
        }
        else if(values['gamer[email]'] == '' || values['gamer[email]'] == "Email") {
          $(".email_error").html(_t('games.enter_email'));
          hasError = true;
        }
        else if(!emailReg.test(values['gamer[email]'])) {
          $(".email_error").html(_t('games.enter_valid_email'));
          hasError = true;
        }
        else if(values['gamer[password]'] == '' || values['gamer[password]'] == "Password") {
          $(".email_error").html(_t('games.enter_password'));
          hasError = true;
        }
        else if(values['gamer[terms_of_service]'] == false) {
          $(".email_error").html(_t('games.enter_tos'));
          hasError = true;
        }
        ...
        if (hasError && cookieError) {
          ...
          var loader = [
            '<div class="title_2 center">'+_t('games.registering')+'</div>',
...
=======
    showAcceptTos : function () {
      ...
      $('#accept_tos_dialog form').submit(function(e){
        ...
        $(".tos_error").hide();
        if(!$('#gamer_terms_of_service').attr('checked')) {
          $(".tos_error").html(_t('games.enter_tos'));
          hasError = true;
        }
        if (hasError) {
          $(".tos_error").show();
          ...
          var loader = [
            '<div class="dialog_title title_2">'+_t('games.updating')+'</div>',
            '<div class="dialog_image"></div>'
          ].join('');
          $("#accept_tos_dialog_content").html(loader);
          $("#accept_tos_dialog_content").parent().animate({ height: "100px", }, animateSpd);
...
>>>>>>> 1d70261625ee91f154994e8ec91eddcef13815d4
            ...
            error: function() {
              ...
              msg = [
                '<div class="dialog_header_wrapper"><div class="dialog_header_right"></div><div class="dialog_header_left"></div><div class="dialog_title title_2">'+_t('games.oops')+'</div></div>',
                '<div class="dialog_content"><div>', error ,'.</div><div id="accept_tos_again"><div class="button grey dialog_button">'+_t('shared.try_again')+'</div></div></div>',
              ].join('');
              $("#accept_tos_dialog_content").html(msg);
              $('#accept_tos_again').click(function(){
                 TJG.ui.showAcceptTos();
    ...
    showDeviceSelection : function(devices, showClose) {
      ...
      if (device_found == false) {
        if (TJG.vars.isIos && TJG.ios_link_device_url) {
          link_device = '<a href="' + TJG.ios_link_device_url + '"><div class="button grey">'+_t('games.connect_my_device')+'</div></a>';
          m =  [
            close,
            '<div class="dialog_header bold">'+_t('games.connect_your_device')+':</div>',
          ...
          link_device = '<a href="' + TJG.android_market_url + '"><div class="button grey">'+_t('games.connect_my_device')+'</div></a>';
          m =  [
            close,
            '<div class="dialog_header bold">'+_t('games.connect_your_android')+':</div>',
            '<div class="dialog_content">',
              '<ul>',
                link_device,
              '</ul>',
          ...
          m =  [
            close,
            '<div class="dialog_header bold">'+_t('games.select_device')+':</div>',
            '<div class="dialog_content">',
              '<ul>',
                a.join(''),
              '</ul>',
        ...
        if (TJG.vars.isAndroid &&  TJG.android_market_url) {
          other = '<a href="' +  TJG.android_market_url + '"><div class="button grey">'+_t('shared.other')+'</div></a>';
        }
        else if (TJG.vars.isIos && TJG.ios_link_device_url) {
          other = '<a href="' +  TJG.ios_link_device_url + '"><div class="button grey">'+_t('shared.other')+'</div></a>';
        }
        m =  [
          close,
          '<div class="dialog_header bold">'+_t('games.select_current_device')+':</div>',
          '<div class="dialog_content">',
            '<ul>',
              d.join(''),
              other,
    ...
    homeInit : function () {
      ...
      function showIntro() {
        ...
        div.id = id;
        div.style.cssText += 'position:absolute;';
        var m =  '<div class="close_button"></div><div class="dialog_content bold">'+_t('games.how_it_works_title')+'</div><div>'+_t('games.how_it_works_body')+'</div>';
        ...
        $("#top_grossing_games_tab").click(function() {
          ...
          if (TJG.topAppOfferWall) {
            ...
            $.ajax({
              ...
              error: function () {
                var m = [
                  '<div>'+_t('games.generic_issue')+'</div>'
                ].join('');
                $("#more_games_content").fadeOut(fadeSpdFast, function () {
