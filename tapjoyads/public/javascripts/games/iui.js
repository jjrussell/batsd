...
function fitToScreen() 
{
	...
	for(var i=1; i<=(sc.length-1); i++) {
		if(document.getElementsByClassName('toolbar')[0] != sc[i])
		{
			if((sc[i].id != '') && (sc[i].id != undefined) && (sc[i].title != '') && (sc[i].title != undefined) && (typeof sc[i] === 'object')) {
				if(window.navigator.standalone===false) 
				{	// for iphone
					if(hasClass(sc[i], 'dialog'))
						sc[i].style.minHeight = (window.innerHeight+60)+'px';
					else
						sc[i].style.minHeight = ((window.innerHeight-toolbarHeight)+60)+'px';
				}
				else
				{
					if(hasClass(sc[i], 'dialog'))
						sc[i].style.minHeight = (window.innerHeight)+'px';
					else {
						if(navigator.userAgent.toLowerCase().search('android') > -1)
							sc[i].style.minHeight = (window.innerHeight+10)+'px';
						else
							sc[i].style.minHeight = (window.innerHeight-toolbarHeight)+'px';
					}
				}
			}
		}
